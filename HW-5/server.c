...
request_type parse_request(recv_buffer *ps) {
  ...
  // root should be used when you've received a complete request, and you know
  // it's for index.html.
  if(strstr(ps->requestbuf, "GET / ") != NULL){
    return root;
  }
  // speak should be used when you've received a complete request, and you know
  // it's a POST message for the resource /speak.
  if(strstr(ps->requestbuf, "POST /speak") != NULL){
    return speak;
  }
  // sse_listen should be used when you've received a complete request, and you
  // know it's for the /listen endpoint.
  if(strstr(ps->requestbuf, "GET /listen") != NULL){
    return sse_listen;
  }
  else 
  {
...
void check_clients(pool *p) {
  ...
  for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
    ...
    if ((connfd > 0) && (FD_ISSET(connfd, &p->ready_set))) {
      ...
      int n = read(p->clientfd[i], &rb->requestbuf[offset], 4096);
      if(n != 0) {
      ...
      switch (parse_request(rb)) {
        ...
        case root:
          ...
          writen(p->clientfd[i], OK, strlen(OK));
          writen(p->clientfd[i], indexhtml, strlen(indexhtml));
        ...
        case speak:
          ...
          for(int j=0; j <= p->maxi; j++)
          {
            ...
            {
              writen(p->clientfd[j], nike, strlen(nike));
            }
          }
          writen(p->clientfd[i], OK, strlen(OK));
          close_and_remove(p, i);
